...
| <strong>smo: *bool, default=True*</strong>                                                                 | Whether to use SMO method, otherwise universal CVXOPT optimizer is used. For `smo=False` the parameters `tau`, `alg`, `ff_caches`, `distance_cache_size`, `max_iter` and `random_seed` are ignored. |
